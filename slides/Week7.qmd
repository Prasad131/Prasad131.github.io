...
Fully developed condition, we evaluate $Gz=\frac{L/D}{Re_D Pr}<<1$
...
$$
\dot{m} c_{p} dT_{m} = h(T_{w}(x)-T_{m}(x)) = q_{w}''dx
$$
...
$q''\geq 22.32 W / m^{2}$, $T_{out} \geq 51.36 ^{0} C$ 
...
Non-dimensionalise the above equation with $r^{*}=\frac{r}{r_{0}}$, $\theta(r^{*})=\frac{T-T_{w}}{T_{mi}-T_{w}}$, and $\frac{u}{u_{m}}(r^{*})=2(1-r^{*2})$
...
$$
\begin{align}
Nu_{H} & = \frac{-\left. \frac{d\theta}{dY} \right|_{Y=1}}{\theta_{m}} \\
...
Answers: $Nu_{H}=Nu_{C}=\frac{35}{8} \approx 4.375$ which is higher than 3.66 (the solution for isothermal no heating scenario)
